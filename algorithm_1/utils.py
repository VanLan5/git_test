...
class ngsimDataset_ZLY(Dataset):


    def __init__(self, mat_file, t_h=30, t_f=50, d_s=1, enc_size = 64, grid_size = (13,3)):
        def trans_tracks_arr2dict(tracks):  # 4列数据[id,帧,x,y]
            veh_id = np.unique(tracks[:,0])
            new_tracks = {}
            for key in veh_id:
                new_tracks[key] = tracks[tracks[:,0]==key][:,1:]
            return new_tracks # 转化为id字典对应3列数据

        self.D = scp.loadmat(mat_file)['traj']  # 所有轨迹数据综合47列
        self.T = trans_tracks_arr2dict(scp.loadmat(mat_file)['tracks'])  # id[帧,x,y]
        self.T_veh_ids = np.unique(list(self.T.keys())) # 所有车辆id
        self.t_h = t_h  # length of track history
        self.t_f = t_f  # length of predicted trajectory
        self.d_s = d_s  # down sampling rate of all sequence 所有序列下的采样率为1
    ...
    # idx是根据batchsize划分数据后得到的索引，最后将data和对应的labels进行一起返回
    def __getitem__(self, idx):

        dsId = self.D[idx, 0].astype(int)
        vehId = self.D[idx, 1].astype(int)
        t = self.D[idx, 2]
        grid = self.D[idx,8:] # dataprocess中计算的数值13*3 = 39个值
        neighbors = []

        # Get track history 'hist' = ndarray, and future track 'fut' = ndarray
        hist = self.getHistory(vehId,t,vehId,dsId) # 获取历史长度序列
        fut = self.getFuture(vehId,t,dsId) # 获取未来长度序列

        # Get track histories of all neighbours 'neighbors' = [ndarray,[],ndarray,ndarray]
        for i in grid:
            neighbors.append(self.getHistory(i.astype(int), t,vehId,dsId)) # 多个周边序列

        # Maneuvers 'lon_enc' = one-hot vector, 'lat_enc = one-hot vector
        lon_enc = np.zeros([2])
        lon_enc[int(self.D[idx, 7] - 1)] = 1 # 现全为[1,0]
        lat_enc = np.zeros([3])
        lat_enc[int(self.D[idx, 6] - 1)] = 1 # 全为[1,0,0]

        return hist,fut,neighbors,lat_enc,lon_enc



    ## Helper function to get track history 获取历史轨迹
    def getHistory(self,vehId,t,refVehId,dsId):
        if vehId == 0:
            ...
            if vehId not in self.T_veh_ids:
                ...
                return np.empty([0,2])
            refTrack = self.T[refVehId]# .transpose()本应是其他车辆3列信息？
            vehTrack = self.T[vehId]# .transpose()本车3列信息
            refPos = refTrack[np.where(refTrack[:,0]==t)][0,1:3] # 取字典对应xy值，只有一行，为了后续减去此基本信息

            if vehTrack.size==0 or np.argwhere(vehTrack[:, 0] == t).size==0:
                 return np.empty([0,2])
            else:
                # np.argwhere返回非0索引，获取此时刻前10帧，所以此前才会去除边缘信息
                stpt = np.maximum(0, np.argwhere(vehTrack[:, 0] == t).item() - self.t_h)  # 22
                enpt = np.argwhere(vehTrack[:, 0] == t).item() + 1 # 43，21行，少一行无法运行
                hist = vehTrack[stpt:enpt:self.d_s,1:3]-refPos # [22:43:1, 1:3] 第22到42行，为历史前21行22-42

            if len(hist) < self.t_h//self.d_s + 1: # 10<10/1
                return np.empty([0,2])
            return hist
    ...
    def getFuture(self, vehId, t, dsId):
        ...
        refPos = vehTrack[np.where(vehTrack[:, 0] == t)][0, 1:3]  # 取字典对应xy值，只有一行，为了后续减去此基本信息
        stpt = np.argwhere(vehTrack[:, 0] == t).item() + self.d_s # 43
        enpt = np.minimum(len(vehTrack), np.argwhere(vehTrack[:, 0] == t).item() + self.t_f + 1) # 63
        fut = vehTrack[stpt:enpt:self.d_s,1:3]-refPos # [43:53:1, 1:3] 第43到62行，为未来20行43-62
        return fut



    ## Collate function for dataloader
    # samples即__getitem__返回值，一个batch后自动运行
    def collate_fn(self, samples):

        # Initialize neighbors and neighbors length batches:
        nbr_batch_size = 0
        for _,_,nbrs,_,_ in samples: # 区域车辆数
            nbr_batch_size += sum([len(nbrs[i])!=0 for i in range(len(nbrs))])
        maxlen = self.t_h//self.d_s + 1
        nbrs_batch = torch.zeros(maxlen,nbr_batch_size,2) # 21**n*2


        # Initialize social mask batch:
        pos = [0, 0]
        mask_batch = torch.zeros(len(samples), self.grid_size[1],self.grid_size[0],self.enc_size) # 512*7*7*64
        mask_batch = mask_batch.byte()


        # Initialize history, history lengths, future, output mask, lateral maneuver and longitudinal maneuver batches:
        hist_batch = torch.zeros(maxlen,len(samples),2)               # 21*512*2
        fut_batch = torch.zeros(self.t_f//self.d_s,len(samples),2)    # 20*512*2
        op_mask_batch = torch.zeros(self.t_f//self.d_s,len(samples),2)# 20*512*2
        lat_enc_batch = torch.zeros(len(samples),3)                   # 512*3
        lon_enc_batch = torch.zeros(len(samples), 2)                  # 512*2
        ...
        for sampleId,(hist, fut, nbrs, lat_enc, lon_enc) in enumerate(samples):
            ...
            op_mask_batch[0:len(fut),sampleId,:] = 1 # 直接赋值？
...
def maskedNLL(y_pred, y_gt, mask):
    ...
    # out = 0.5 * torch.pow(ohr, 2) * (torch.pow(sigX, 2) * torch.pow(x - muX, 2) + torch.pow(sigY, 2) * torch.pow(y - muY, 2) - 2 * rho * torch.pow(sigX, 1) * torch.pow(sigY, 1) * (x - muX) * (y - muY)) - torch.log(sigX * sigY * ohr) + 183.79 - 51.60
    acc[:,:,0] = out
    acc[:,:,1] = out
    acc = acc*mask
